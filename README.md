...
城市、角色、材质和技能视觉效果是原创程序化素材。项目重建城市主题和循环交互，不包含原版客户端、商业游戏素材、多人联网服务或原版地图的逐坐标复制。威力统计用于比较技能循环，不是装备面板下的实际 DPS。具体战斗规则和简化边界见 [RULES.md](docs/RULES.md)。
...
本项目不自动下载或提取 FFXIV 客户端资产。若后续引入有使用权限的模型，可替换 `world/actors.js` 与场景素材模块，保留现有战斗引擎和界面。
