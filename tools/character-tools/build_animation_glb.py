...
def parse_skeleton_dump(path):
    ...
    if not skeleton_starts:
        raise ValueError("no hkaSkeleton found")
    start = skeleton_starts[1 if len(skeleton_starts) > 1 else 0]
...
def build_glb(skeleton, tracks, destination, clip_name):
    nodes = []
    ...
    document = {
        ...
        "extras": {
            "sourcePapAnimation": clip_name,
            "sourceSkeleton": "FFXIV SKLB direct skeleton",
...
def main():
    parser = argparse.ArgumentParser(description="Build a Three.js-playable animation GLB from SKLB/PAP decoder output")
    parser.add_argument("skeleton_dump")
    parser.add_argument("tracks_csv")
    parser.add_argument("output")
    parser.add_argument("clip_name")
    args = parser.parse_args()
    skeleton = parse_skeleton_dump(Path(args.skeleton_dump))
    ...
    build_glb(skeleton, tracks, destination, args.clip_name)
