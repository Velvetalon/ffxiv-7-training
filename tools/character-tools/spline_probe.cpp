...
int main(int argc, char **argv) {
    if (argc != 4) {
        std::cerr << "usage: spline_probe <animation-tagfile> <havok-index> <output-csv>\n";
        return 2;
    }
    try {
        ...
        std::unordered_set<HKXStruct *> visited;
        HKXStruct *container = findClass(root, "hkaAnimationContainer", visited);
        if (!container) throw std::runtime_error("animation container not found");
        const size_t selected = static_cast<size_t>(std::stoul(argv[2]));
        auto &animationValues = std::get<HKXArray>(container->fields.at("animations")).values;
        auto &bindingValues = std::get<HKXArray>(container->fields.at("bindings")).values;
        if (selected >= animationValues.size()) throw std::runtime_error("animation index out of range");
        HKXStruct *animation = std::get<HKXStructRef>(animationValues[selected]).get();
        HKXStruct *binding = nullptr;
        for (auto &item : bindingValues) {
            auto candidate = std::get<HKXStructRef>(item).get();
            auto target = std::get<HKXStructRef>(candidate->fields.at("animation")).get();
            if (target == animation) { binding = candidate; break; }
        }
