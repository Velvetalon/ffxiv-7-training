export class TeleportController {
  ...
  start(id, immediate = false) {
    ...
    if (immediate) this.finish(id);
    else {
      this.cast = { id, name: '传送', remaining: 5, total: 5 };
      this.onStart();
    }
    return { ok: true };
  }
  ...
  finish(id) { this.cast = null; this.onFinish(id); }
  update(dt, moving) {
    if (!this.cast) return;
    if (moving) { this.cancel(); return; }
    this.cast.remaining -= dt;
    if (this.cast.remaining <= 0) this.finish(this.cast.id);
  }
}
