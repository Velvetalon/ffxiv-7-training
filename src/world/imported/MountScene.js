...
export function mountEncounter(world, loaded, navigation) {
  ...
  navigation.height = y;
  const dummyPoint = navigation.nearestWalkable(x + 1,z + 8,20,y);
  if (!dummyPoint) throw new Error('无法在真实碰撞面上找到木人位置');
  const spawn = navigation.nearestWalkable(dummyPoint.x,dummyPoint.z + 7,12,dummyPoint.y);
  if (!spawn) throw new Error('无法在真实碰撞面上找到出生点');
  ...
  const dummy = createDummy();dummy.rotation.y=Math.PI;
  ...
  return {
    ...
    image:`/extracted/${world.sceneId}/map.png`,
