import { sceneDisplayName, sceneDisplayRegion, sceneNameSource, sceneRegionSource } from './sceneCatalog.js';
...
export async function loadSceneCatalog() {
  ...
  const scenes = Object.entries(active.scenes || {}).map(([id, record]) => ({
    id, name: sceneDisplayName(record, defaults.get(id)?.name || id),
    nameSource: sceneNameSource(record),
    en: record.en || defaults.get(id)?.en || id.toUpperCase(),
    region: sceneDisplayRegion(record, defaults.get(id)?.region || '大世界'),
    regionSource: sceneRegionSource(record),
