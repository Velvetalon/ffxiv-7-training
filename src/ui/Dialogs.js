...
function openTeleport() {
  ...
  openModal('传送', 'AETHERYTE NETWORK', `<div class="teleport-layout"><nav class="teleport-regions"><b>已开放地区 <span>${SCENES.length}</span></b><button class="selected" data-region="all">${icon('globe-2')}全部地区</button>${regions.map(region => `<button data-region="${escape(region)}">${icon('map-pin')}${escape(region)}</button>`).join('')}</nav><div class="destinations"><div class="destination-heading"><span>以太之光</span><span>状态</span></div>${SCENES.map(scene => `<button class="destination ${scene.id === getSceneId() ? 'current' : ''}" data-teleport="${scene.id}" data-scene-region="${escape(scene.region)}"><span class="destination-art ${scene.id}">${icon(scene.id === 'gridania' ? 'tree-pine' : 'anchor')}</span><span class="destination-label"><small>${escape(scene.region)}</small><strong>${escape(scene.name)}</strong><em>${escape(scene.en)}</em></span><span class="destination-status">${scene.id === getSceneId() ? '当前地区' : '已共鸣'}${icon(scene.id === getSceneId() ? 'check' : 'chevron-right')}</span></button>`).join('')}<div class="teleport-footer"><span>${icon('diamond')}以太之光已共鸣</span><b>费用 0 Gil</b></div></div></div>`, 'teleport-modal');
}
...
function openBook(filter = '') {
  ...
  openModal('技能一览', `${combat.getState().jobId} · LEVEL 100`, `
    ...
    <div class="skillbook">${actions.map(action => `<button class="book-skill" data-inspect="${escape(action.id)}" data-search="${escape(`${action.name} ${action.en || ''}`.toLowerCase())}"><span style="--skill-color:${action.color || '#c1bd9e'}" class="book-icon">${icon(action.icon || 'sparkles')}</span><span><strong>${escape(action.name)}</strong><small>${action.gcd ? '魔法 / 战技' : '能力'} · ${action.cast ? `${action.cast}s 咏唱` : '即时'}</small></span><em>${action.potency || '—'}</em></button>`).join('')}</div>`, 'book-modal');
}
...
function openHelp() {
  openModal('演武指南', 'FIELD GUIDE', `<div class="help-body">
    ...
    <div class="help-grid"><span>移动</span><b>W A S D / 方向键</b><span>旋转 / 缩放视角</span><b>鼠标拖动 / 滚轮</b><span>跳跃</span><b>Space</b><span>选择最近木人</span><b>Tab / 点击木人</b><span>使用技能</span><b>1–0、−、=</b><span>第二行技能</span><b>Shift + 数字键</b><span>打开传送 / 地图 / 技能</span><b>T / M / P</b><span>关闭窗口 / 取消目标</span><b>Esc</b></div>
    <p>鼠标悬停技能可查看效果与条件。亮金色边框代表已触发技能；复唱与资源不足会显示在技能栏。点击技能栏右侧箭头切换全部技能。</p>
    <p>近战技能需要靠近木人；移动会中断咏唱。传送需要 5 秒，战斗中请先重置练习。范围攻击可在设置中调整目标数。</p>
    <p>左侧统计以技能威力计算。随时重置即可重新练习，也可导出战斗记录。</p>
  </div>`, 'help-modal');
}
return { openTeleport, openBook, inspectSkill, openSettings, openMap, openDialogue, openHelp, closeModal, get active() { return modal; } };
