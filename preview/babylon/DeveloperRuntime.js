...
import { getEnvironmentProfile } from '../../src/world/environment/EnvironmentProfiles.js';
import { instantiateNativeAsset, loadNativeContainer, loadRetargetedAnimation } from './character/NativeAssetLoader.js';
...
export class DeveloperRuntime {
  constructor(options = {}) {
    const {
      ...
      preferences = null,
      changeMap = null,
      importAppearance = null,
      reloadCharacter = null,
  ...
  getSnapshot() {
    ...
    return {
      ...
      environment: {
        ...
        profiles: this.sceneList().map(entry => ({ id: entry.id, name: entry.name })).slice(0, LIST_LIMIT),
  ...
  async run(command, args = {}) {
    ...
    try {
      switch (name) {
        ...
        case 'audio.bgm': case 'audio.bgm.play': return this.playBgm(args);
        case 'audio.bgm.stop': return this.stopAudio({ ...(isObject(args) ? args : {}), bgmOnly: true });
        case 'audio.sfx': case 'audio.sfx.play': return this.playSfx(args);
        case 'audio.sfx.stop': case 'audio.stop': return this.stopAudio(args);
  ...
  update(dt = 0) {
    ...
    this.preview?.animationGroups?.forEach(group => {
      if (group?.isPlaying && group.speedRatio !== undefined) group.speedRatio = group.speedRatio;
    });
    ...
    if (lock && character?.animation) {
      ...
      if (character.animation.actionRemaining <= 0 && !lock.pending) this.animationPreview = null;
  ...
  clearPreview() {
    ...
    try { preview.dispose?.(); } catch { /* Native preview teardown is best effort. */ }
    if (!preview.dispose) {
      try { preview.root?.dispose?.(false, true); } catch { /* The loader can own child disposal. */ }
    }
  ...
  async teleport(args = {}) {
    ...
    if (!player?.position || !position) return { ok: false, reason: 'Teleport position callback is unavailable' };
    setVector(player.position, position);
    return { ok: true, position: vectorSnapshot(player.position) };
  }
  ...
  triggerSkill(args = {}) {
    ...
    if (definition && this.world?.character?.actions?.trigger) this.world.character.actions.trigger(event, definition, 'resolved');
    else if (typeof this.world?.effect === 'function') this.world.effect(event);
  ...
  async spawnMountPreview(args = {}) {
    ...
    if (!definition?.model || !this.assetRuntime?.load || !scene) return { ok: false, reason: 'Mount preview resource is unavailable' };
    this.clearPreview();
    const generation = ++this.previewGeneration;
    const loaded = new Map();
    try {
      const source = await loadNativeContainer(scene, this.assetRuntime, definition.model, { retain: true, priority: 0 });
      loaded.set(definition.model, source);
      if (generation !== this.previewGeneration) {
        source.dispose();
        return { ok: false, stale: true };
      }
      if (!source) throw new Error(`Mount model has no native container: ${definition.model}`);
      const root = new TransformNode('developer-mount-preview', scene);
      root.metadata = { ...(root.metadata || {}), developerPreview: true };
      const instance = instantiateNativeAsset(source, scene, { name: `developer-mount-${definition.id || definition.model}`, parent: root, cloneMaterials: true });
      const scale = Number(definition.modelScales?.[this.world?.character?.state?.appearance?.modelFamily] ?? definition.scale ?? 1);
      if (Number.isFinite(scale) && scale > 0) root.scaling.copyFromFloats(scale, scale, scale);
      ...
      const x = Number(position?.x ?? (player?.position?.x ?? 0) + (hasPosition ? 0 : 3)) || 0;
      const z = Number(position?.z ?? player?.position?.z ?? 0) || 0;
      const floor = !Number.isFinite(Number(position?.y)) ? this.world?.navigation?.surfaceAt?.(x, z, player?.position?.y, 5) : null;
      const y = Number(position?.y ?? floor?.height ?? player?.position?.y ?? 0) || 0;
      root.position.copyFromFloats(x, y, z);
      root.rotation.y = Number(position?.heading ?? args.heading ?? 0) || 0;
      const state = cleanId(args.state || 'idle') || 'idle';
      const retargeted = await Promise.all(Object.entries(definition.animations || {}).map(async ([animationState, resourceId]) => {
        const group = await loadRetargetedAnimation(scene, this.assetRuntime, resourceId, instance.root, {
          name: animationState,
      ...
      if (generation !== this.previewGeneration) {
        for (const group of retargeted) group?.dispose?.();
        instance.dispose();
        return { ok: false, stale: true };
      }
      const animationGroups = animationGroupsFor(instance, retargeted, definition, state);
      playAnimationGroups(animationGroups, state);
      loaded.clear();
      this.preview = {
        kind: 'mount',
        id: definition.id || definition.model,
        root,
        animationGroups,
        resources: new Set(),
        state,
  ...
  setEnvironmentTime(args = {}) {
    const hour = Number(isObject(args) ? args.hour ?? args.value : args);
    if (!Number.isFinite(hour) || !this.world?.worldTime?.setHour) return { ok: false, reason: 'World time is unavailable' };
    const result = this.world.worldTime.setHour(hour, args.day);
    this.manualOverrides.time = true;
    return { ok: true, time: compactTime(result) };
  }

  setEnvironmentPause(args = {}) {
    const paused = Boolean(isObject(args) ? args.paused ?? args.value : args);
    if (!this.world?.worldTime?.setState) return { ok: false, reason: 'World time is unavailable' };
    const result = this.world.worldTime.setState({ paused });
    this.manualOverrides.pause = true;
    return { ok: true, time: compactTime(result) };
  }
  ...
  sceneList() {
    const source = this.scenes.length ? this.scenes : this.assets?.config?.maps ? Object.entries(this.assets.config.maps).map(([id, entry]) => ({ id, ...entry })) : [];
    const signature = source.slice(0, LIST_LIMIT).map(scene => scene.id).join('|');
    ...
    this.listCache.scenes = source.slice(0, LIST_LIMIT).map(scene => ({ id: scene.id, name: scene.name || scene.en || scene.fullSceneName || scene.id, en: scene.en || null, region: scene.region || null }));
  ...
  mountSnapshot(mount) {
    ...
    return state ? { ...state, id: state.mountId || definition?.id || null, name: definition?.name || null, model: definition?.model || null, state: state.isMounted ? state.movementMode : 'dismounted' } : null;
  }
  ...
  characterSnapshot(character) {
    ...
    for (const mesh of model?.getChildMeshes?.(false) || []) {
      ...
      for (const entry of materials) {
        const id = entry?.metadata?.materialId || entry?.metadata?.id || entry?.name;
        if (id && materialIDs.size < 64) materialIDs.add(String(id));
      }
    }
  ...
  debugSnapshot(world) {
    ...
    return {
      ...
      renderer: engine ? { fps: finite(engine.getFps?.()), drawCalls: engine._drawCalls?.current ?? null, activeMeshes: scene?.getActiveMeshes?.().length ?? null } : null,
    };
  }
  ...
  manifest() { return this.assets?.manifest || this.assetRuntime?.manifest || {}; }
  resolveMount(value) { return value && typeof value === 'object' ? value : this.mountDefinitionList().find(item => item.id === value) || null; }
  mountDefinitionList() { return Array.isArray(this.assets?.mounts) ? this.assets.mounts : Object.values(this.manifest().mounts || {}); }
  resolveSkill(value) { return value && typeof value === 'object' ? value.id ? value : { ...value, id: value.skillId } : this.world?.skillDefinitions?.get?.(value) || this.world?.skillDefinitions?.get?.(String(value)) || (value !== undefined ? (this.assets?.skills?.[value] || this.manifest().skills?.[value]) : null); }
  invokeCallback(callback, args) { return Promise.resolve(callback(args)).then(result => result === false ? { ok: false, reason: 'Character operation failed' } : { ok: true, result }); }
}

function animationGroupsFor(instance, retargeted, definition, state) {
  const all = [...(instance?.animationGroups || instance?.groups || []), ...(retargeted || [])];
  const requested = definition.animations?.[state];
  if (!requested) return all;
  const matches = all.filter(group => group.name === requested || group.metadata?.resourceId === requested || group.metadata?.id === requested);
  return matches.length ? matches : all;
}

function playAnimationGroups(groups, state) {
  const candidates = groups.filter(Boolean);
  for (const group of candidates) {
    const matchesState = group.name?.toLowerCase?.().includes(state.toLowerCase());
    if (matchesState || candidates.length === 1) group.start?.(true, 1, group.from, group.to, false);
  }
}
...
function setVector(target, source) {
  ...
  if (target.copyFromFloats) target.copyFromFloats(x, y, z);
  else target.copyFrom(new Vector3(x, y, z));
}
...
function audioEntries(table, runtime, skills = null) { const result = []; const seen = new Set(); const add = (id, value, skillId = null) => { const resourceId = resourceIdOf(value); if (!resourceId) return; const key = `${id}:${resourceId}`; if (seen.has(key) || result.length >= AUDIO_LIMIT) return; seen.add(key); result.push({ id, skillId, resourceId, resource: resourceInfo(resourceId, runtime) }); }; for (const [id, value] of Object.entries(table || {})) add(id, value); for (const [skillId, skill] of Object.entries(skills || {})) { add(skillId, skill?.soundId, skillId); for (const [index, cue] of (skill?.soundEvents || []).entries()) add(`${skillId}:${index}`, cue?.resourceId || cue?.soundId, skillId); } return result; }
