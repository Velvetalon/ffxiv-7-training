export class ActionRuntime {
  ...
  trigger(event, definition, phase) {
    ...
    if (!duplicateBurst) {
      ...
      this.triggered.set(event.actionId, { phase, time: this.time, until: this.time });
      this.lastAction = event.actionId;
      this.lastPlayed = this.time;
    }
    if (phase === 'resolved' && event.type !== 'field') {
      this.effects?.play(event, this.targetPosition(), this.character.state.jobId);
    }
  }
