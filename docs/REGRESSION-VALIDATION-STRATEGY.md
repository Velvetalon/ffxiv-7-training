...
> 已被 [Fast Validation / 快速验收](FAST-VALIDATION.md) 取代为日常默认策略。本文及 `regress:*` 工具仅供明确要求的深度诊断/回归使用，不得在普通开发或发布时自动叠加执行。

以下保留深度回归的设计参考。地图数量可以继续增长，主 Agent 的阅读与分析成本应主要取决于异常数量，而不是地图总数。
